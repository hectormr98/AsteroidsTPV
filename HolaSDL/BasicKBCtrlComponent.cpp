...
BasicKBCtrlComponent::BasicKBCtrlComponent(SDL_Keycode left,
		SDL_Keycode right, SDL_Keycode up, SDL_Keycode down, SDL_Keycode stop, double angle, SDL_Keycode RotRight, SDL_Keycode RotLeft) :
		left_(left), right_(right), up_(up), down_(down), stop_(stop), InputComponent(angle, RotRight, RotLeft){
}
...
void BasicKBCtrlComponent::handleInput(GameObject* o, Uint32 time,
	...
	if (event.type == SDL_KEYDOWN) {
		...
		else InputComponent::handleInput(o, time, event);
	}
